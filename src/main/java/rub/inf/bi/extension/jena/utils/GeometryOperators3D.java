...
public class GeometryOperators3D {
	...
	public static ArrayList<Vector3D> intersection3D(Polygon planeFaceArea, LineString line) {
		...
		for(int lI = 1; lI < line.getCoordinates().length; lI++) {
	    	...
	    	if(interSectionPoint != null) { 
	    		if(contains3D(tempA, tempB, interSectionPoint) && contains3D(planeFaceArea, interSectionPoint)) { //TODO check if point is on plane
	...
	public static boolean contains3D(Polygon mesh, Vector3D point) {
		...
		//TODO: accept complex areas, currently only triangles and rectangles are accepted
	...
	public static boolean contains3D(Vector3D trianglePointA, Vector3D trianglePointB, Vector3D trianglePointC, Vector3D point) {
	...
	public static boolean contains3D(Vector3D lineStart, Vector3D lineEnd, Vector3D point) {
		...
		return (distance > (sumDistance - GeometryOperators3D.TOLERANCE) && distance < (sumDistance + GeometryOperators3D.TOLERANCE));
	}
	 ...
	 * </p>
	 * 
	 * @param triangleA - The first triangle as Polygon object (must contain a coordinate loop of exactly 4 points!)
	 * @param triangleB - The second triangle as Polygon object (must contain a coordinate loop of exactly 4 points!)
	 * @return ArrayList containing all intersecting points as Vector3D. Result is an empty List if no intersection exists.
	 */
	...
	/***
	 * Finds the intersecting points of a line and the edges of a Polygon face in 3D.
	 * 
	 * <p>
	 * Example: <br>
	 * <br>
	 ...
	 * </p>
	 * 
	 * @param source - The line as Line object that is expected to have an intersection with the mesh edges
