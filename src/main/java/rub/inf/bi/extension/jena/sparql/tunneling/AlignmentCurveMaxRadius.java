...
public class AlignmentCurveMaxRadius extends FunctionBase1 {
		    ...
		    if (geom1 instanceof LineString) {
		    	...
		    	for(int i = 2; i < coordinates.length; i++) {
		    		...
		    		if(!Double.isNaN(curvature)) {
		    			if(curvature > foundCurvature) {
			    			foundCurvature = curvature;
			    		}
		    		}
		    		
		    	}
		    }
		    return NodeValue.makeDouble(foundCurvature);
		}
